...
bool PolishNotation(int i, Lex::LEX& lex)
{
	...
	for (i; lex.lextable.table[i].lexema != LEX_SEMICOLON; i++, countLex++)
	{
		...
		{
			if (lex.lextable.table[i-1].lexema == LEX_ID/*lex.idtable.table[lex.lextable.table[i - 1].idxTI].idtype == IT::F || lex.idtable.table[lex.lextable.table[i - 1].idxTI].idtype == IT::SF*/)
			{
		...
		case LEX_PLUS: case LEX_MINUS: case LEX_STAR: case LEX_DIRSLASH: case LEX_PROC:	// если знак оператора
		{
